...
async function loadPosts(page = 1, filters = {}) {
  ...
  try {
    ...
    if (collabType && collabType !== 'all') {
      query = query.eq('type', collabType);
    }
    
    // Apply skills filter
    if (skills.length > 0) {
      const skillConditions = skills.map(skill => `tags.cs.{${skill}}`).join(',');
      query = query.or(skillConditions);
    }
    
    // Apply experience level filter (assuming tags contain experience levels)
    if (experience.length > 0) {
      const expConditions = experience.map(exp => `tags.cs.{${exp}}`).join(',');
...
function getTopFilters() {
  const filters = {
    collabType: document.querySelector('input[name="collab-type"]:checked')?.value || 'all',
    skills: [],
    experience: [],
  ...
  document.querySelectorAll('input[name="experience"]:checked').forEach(checkbox => {
  ...
  if (posts.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <p>Try adjusting your search or filter criteria, or be the first to share a collaboration opportunity!</p>
      </div>
    `;
    return;
  }
...
async function createPostCard(post) {
  ...
  card.innerHTML = `
    ...
    <div class="post-card-footer">
      ...
      <div class="post-card-actions">
        ...
        ` : ''}
        <a href="/view-post.html?id=${post.id}&type=collab" class="view-btn">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M15 3h6v6"/>
            <path d="M10 14L21 3"/>
  ...
  card.addEventListener('click', (e) => {
    ...
    window.location.href = `/view-post.html?id=${post.id}&type=collab`;
  });
...
function setupEventListeners() {
  ...
  document.addEventListener('change', (e) => {
    if (e.target.matches('input[name="collab-type"], input[name="skill"], input[name="experience"], input[name="date-from"], input[name="date-to"], input[name="views-min"], input[name="views-max"]')) {
      performSearch();
    }
  });
...
function updateStats() {
  ...
  if (totalPosts === 0) {
    ...
    statsElement.innerHTML = `<p>Showing ${startIndex}-${endIndex} of ${totalPosts} collaboration posts</p>`;
  }
}
